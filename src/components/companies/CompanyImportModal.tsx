...
export const CompanyImportModal: React.FC<CompanyImportModalProps> = ({ onClose }) => <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
  <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
    <div className="flex items-center justify-between border-b border-slate-100 pb-4"><div className="flex items-center gap-2.5"><div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-2.5 text-indigo-600"><FileSpreadsheet className="h-5 w-5" /></div><div><h3 className="text-base font-bold text-slate-900">Importar empresas</h3><p className="text-xs text-slate-500">Importação CSV com persistência real</p></div></div><button onClick={onClose} className="rounded-full bg-slate-100 p-1.5 text-slate-500" aria-label="Fechar"><X className="h-4 w-4" /></button></div>
    <div className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center"><FileSpreadsheet className="mx-auto h-8 w-8 text-slate-400" /><p className="mt-3 text-sm font-semibold text-slate-800">Importação CSV indisponível nesta versão</p><p className="mt-1 text-xs leading-5 text-slate-500">Nenhuma empresa será criada localmente. Use o cadastro manual até a importação persistente ser disponibilizada.</p></div>
