...
function mapTask(row: Row, ownerName = row.owner_id || "Sem responsÃ¡vel"): TaskItem {
...
async function ownerNames(client: Client, ids: (string | null)[]) {
  ...
  return new Map((data || []).map((item) => [item.id, item.full_name || item.id]));
}
...
export async function archiveTask(client: Client, organizationId: string, id: string) { const { data, error } = await client.from("tasks").update({ archived_at: new Date().toISOString() }).eq("organization_id", organizationId).eq("id", id).select("*").single(); if (error) throw error; return mapTask(data); }
