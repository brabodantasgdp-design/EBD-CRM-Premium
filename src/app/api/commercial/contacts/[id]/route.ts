...
import { updateContact } from "../../../../../lib/crm/commercial";
...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    const contact = await updateContact(supabase, organization.id, id, body);
    return NextResponse.json({ contact });
  } catch {
    return NextResponse.json({ error: "Não foi possível atualizar o contato" }, { status: 400 });
  }
}
