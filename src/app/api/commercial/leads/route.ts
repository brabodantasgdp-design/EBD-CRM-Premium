...
export async function GET() {
  ...
  try { return NextResponse.json({ leads: await listLeads(supabase, organization.id) }); }
  catch { return NextResponse.json({ error: "Não foi possível carregar os leads" }, { status: 500 }); }
}
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json({ lead: await createLead(supabase, organization.id, user.id, body) }, { status: 201 }); }
  catch { return NextResponse.json({ error: "Não foi possível criar o lead" }, { status: 400 }); }
}
