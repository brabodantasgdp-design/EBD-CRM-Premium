...
def run():
    ...
    with sync_playwright() as playwright:
        ...
        check("company_refresh", page.get_by_text(company["name"], exact=True).count() > 0)
        ...
        if len(open_deals) == 2:
            ...
            page.get_by_text("Alterar Etapa", exact=True).click(); page.wait_for_timeout(300)
            ...
            stage_button = page.locator("div.fixed.bottom-6").get_by_role("button", name=target_stage["name"], exact=True)
        ...
        check("kpi_open", money(expected_open) in body, {"expected": expected_open})
        check("kpi_forecast", money(expected_forecast) in body, {"expected": expected_forecast})
        check("kpi_won", str(expected_won) in body, {"expected": expected_won})
